...
(function(global){
  ...
  function request_script(chain_opts, script_obj, registry_item, onload, preload_this_script) {
    ...
    setTimeout(function(){
      ...
      if (preload_this_script) {//当请求的脚本文件是否进行预加载：1、需要预加载 2、浏览器支持预加载
        // real script preloading?
        if (real_preloading) {//如果支持真正的预加载
          /*!START_DEBUG*/
          if (chain_opts[_Debug]) log_msg("start script preload: "+src);
          /*!END_DEBUG*/
          registry_item.elem = script;
          if (explicit_preloading) { // explicit preloading (aka, Zakas' proposal)
            //支持通过设置script标签的preload属性，实现script的预加载，以及分离加载和执行
            //Nicholas C. Zakas大神的美好愿望，尚未有浏览器支持
            ...
            script.onreadystatechange = function(){
              //其实就是指IE浏览器，假设指定了script元素的src属性，IE浏览器里会立即加载
        ...
        // same-domain and XHR allowed? use XHR preloading
          ...
          //由于上个判断已经将IE无情地抛弃在这个条件分支之外，所以大胆地用 new XMLHttpRequest()吧
        ...
        // as a last resort, use cache-preloading
          ...
          //最无奈的后招，Cache Trick，新版chromei已经不支持
          /*!START_DEBUG*/
      ...
      // use async=false for ordered async? parallel-load-serial-execute http://wiki.whatwg.org/wiki/Dynamic_Script_Execution_Order
        ...
        //如果能够通过script元素的async属性来强制并行加载的脚本顺序执行
        //kyle大神着力推进的提案，目前已被html5小组接受并放入草案
